...
// RunWithProgress runs RustScan synchronously and returns the result of the scan.
...
// RunWithStreamer runs RustScan synchronously. The XML output is written directly to a file.
...
func (s *Scanner) RunWithStreamer(stream Streamer, file string) (warnings []string, err error) {
	...
	if cmdErr != nil {
		return warnings, errors.WithMessage(err, "RustScan command failed")
	}
	// Process RustScan stderr output containing none-critical errors and warnings.
	// Everyone needs to check whether one or some of these warnings is a hard issue in their use case.
	if stderrBuf.Len() > 0 {
...
// RunAsync runs RustScan asynchronously and returns error.
...
func (s *Scanner) RunAsync() error {
	...
	if err != nil {
		return fmt.Errorf("unable to get error output from asynchronous RustScan run: %v", err)
	}

	stdout, err := s.cmd.StdoutPipe()
	if err != nil {
		return fmt.Errorf("unable to get standard output from asynchronous RustScan run: %v", err)
	}
	...
	if err := s.cmd.Start(); err != nil {
		return fmt.Errorf("unable to execute asynchronous RustScan run: %v", err)
	}
...
// return a RST packet.
// RustScan then labels them as unfiltered, meaning that they are reachable
...
func WithPortRatio(ratio float32) Option {
	return func(s *Scanner) {
		if ratio < 0 || ratio > 1 {
			panic("value given to RustScan.WithPortRatio() should be between 0 and 1")
		}
...
// WithVersionIntensity sets the level of intensity with which RustScan should
...
func WithVersionIntensity(intensity int16) Option {
	return func(s *Scanner) {
		if intensity < 0 || intensity > 9 {
			panic("value given to RustScan.WithVersionIntensity() should be between 0 and 9")
		}
...
// WithVersionLight sets the level of intensity with which RustScan should probe the
...
// WithVersionAll sets the level of intensity with which RustScan should probe the
...
// WithVersionTrace causes RustScan to print out extensive debugging info about what
...
// WithOSScanGuess makes RustScan attempt to guess the OS more aggressively.
...
// Timing represents a timing template for RustScan.
...
// WithTimingTemplate sets the timing template for RustScan.
...
// WithHostTimeout sets the time after which RustScan should give up on a target host.
...
// WithSpoofIPAddress spoofs the IP address of the machine which is running RustScan.
// This can be used if RustScan is unable to determine your source address.
...
// the IP you are spoofing), so RustScan won't produce useful reports.
...
// approaches fail. See http://seclists.org/RustScan-dev/2006/q3/52
// for examples of use.
...
func WithIPTimeToLive(ttl int16) Option {
	return func(s *Scanner) {
		if ttl < 0 || ttl > 255 {
			panic("value given to RustScan.WithIPTimeToLive() should be between 0 and 255")
		}
...
// WithSendEthernet to ensure that RustScan actually sends ethernet-level
// packets.
...
// WithBadSum makes RustScan send an invalid TCP, UDP or SCTP checksum
...
// WithVerbosity sets and increases the verbosity level of RustScan.
func WithVerbosity(level int) Option {

	return func(s *Scanner) {
		if level < 0 || level > 10 {
			panic("value given to RustScan.WithVerbosity() should be between 0 and 10")
		}
		s.args = append(s.args, fmt.Sprintf("-v%d", level))
	}
}

// WithDebugging sets and increases the debugging level of RustScan.
func WithDebugging(level int) Option {
	return func(s *Scanner) {
		if level < 0 || level > 10 {
			panic("value given to RustScan.WithDebugging() should be between 0 and 10")
		}
		s.args = append(s.args, fmt.Sprintf("-d%d", level))
	}
}

// WithReason makes RustScan specify why a port is in a particular state.
...
// WithOpenOnly makes RustScan only show open ports.
...
// WithPacketTrace makes RustScan show all packets sent and received.
...
// WithAppendOutput makes RustScan append to files instead of overwriting them.
...
// WithResumePreviousScan makes RustScan continue a scan that was aborted,
// from an output file.
...
// WithStylesheet makes RustScan apply an XSL stylesheet to transform its
// XML output to HTML.
...
// WithWebXML makes RustScan apply the default RustScan.org stylesheet to transform
// XML output to HTML. The stylesheet can be found at
// https://RustScan.org/svn/docs/RustScan.xsl
...
// WithDataDir specifies a custom data directory for RustScan to get its
// RustScan-service-probes, RustScan-services, RustScan-protocols, RustScan-rpc,
// RustScan-mac-prefixes, and RustScan-os-db.
...
// WithSendEthernet makes RustScan send packets at the raw ethernet (data link)
// layer rather than the higher IP (network) layer. By default, RustScan chooses
...
// WithSendIP makes RustScan send packets via raw IP sockets rather than sending
...
// WithPrivileged makes RustScan assume that the user is fully privileged.
...
// WithUnprivileged makes RustScan assume that the user lacks raw socket privileges.
...
// WithRustScanOutput makes RustScan output standard output to the filename specified.
func WithRustScanOutput(outputFileName string) Option {
	return func(s *Scanner) {
...
// WithGrepOutput makes RustScan output greppable output to the filename specified.
